...
-- for a fresh project. Sets up the full schema: items table (opportunities,
-- courses, roadmaps), row-level security, and the avatars storage bucket.
...
-- Avatars: a public storage bucket with per-user upload/update/delete access,
-- so users can set a profile picture. First/last name live on the auth
-- user's metadata directly (no table needed for those).
